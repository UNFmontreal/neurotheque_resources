...
class Pipeline:
    ...
    def __init__(self, config_file="config/pipeline_config.yaml", config_dict=None):
        ...
        self.project_root = (
            self.config_file_path.parent.parent
        )  # Assuming config file is in a 'config' directory
    ...
    def process_single_file(self, file_path, steps_def, mode, start_from_step):
        ...
        self.data = None
        skip_index = self._determine_start_index(
            steps_def, mode, start_from_step, sub_id, ses_id, task_id, run_id
        )
    ...
    def _determine_start_index(
        self, steps_def, mode, start_from_step, sub_id, ses_id, task_id, run_id
    ):
        if start_from_step:
            skip_index = self._find_step_index(steps_def, start_from_step)
            if skip_index is not None:
                logging.info(
                    f"Starting from step {start_from_step} (index {skip_index})"
                )
                return skip_index - 1
            else:
                logging.warning(
                    f"Step '{start_from_step}' not found. Starting from beginning."
                )
                return None
    ...
    def _auto_save_data(self, context):
        ...
        save_params = {**context, "step_name": "autosave", "paths": self.paths}
    ...
    def _find_latest_checkpoint(self, sub_id, ses_id, task_id=None, run_id=None):
        ...
        checkpoint_files = sorted(
            sub_path.glob(pattern), key=lambda x: x.stat().st_mtime, reverse=True
        )
