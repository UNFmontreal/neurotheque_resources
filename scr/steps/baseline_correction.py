...
class BaselineCorrectionStep(BaseStep):
    ...
    def __init__(self, params=None):
        ...
        logging.info(
            f"[BaselineCorrectionStep] Initialized with method: {self.params['method']}"
        )
    ...
    def run(self, data):
        ...
        if not isinstance(data, mne.Epochs):
            raise ValueError(
                "[BaselineCorrectionStep] Data must be an mne.Epochs object."
            )
        ...
        plot_title = self.params.get(
            "plot_title", "Comparison of Baseline Correction Methods"
        )
        ...
        if interactive:
            try:
                ...
                if (
                    ...
                    logging.info(
                        f"[BaselineCorrectionStep] Switching matplotlib backend from {original_backend} to TkAgg for interactive plotting"
                    )
                    matplotlib.use("TkAgg")
                    plt.ion()  # Turn on interactive mode
            except Exception as e:
                logging.warning(
                    f"[BaselineCorrectionStep] Could not switch to interactive backend: {e}"
                )
        ...
        # Log chosen methods
        logging.info(
            f"[BaselineCorrectionStep] Applying baseline correction methods: {methods}"
        )
        ...
        if "classic" in methods:
            logging.info(
                "[BaselineCorrectionStep] Applying classic baseline subtraction..."
            )
        ...
        if "regression" in methods:
            logging.info(
                "[BaselineCorrectionStep] Applying mass-univariate regression (least squares)..."
            )
            ...
            for ch in range(n_ch):
                ...
                for t in range(n_times):
                    ...
                    try:
                        ...
                        logging.warning(
                            f"[BaselineCorrectionStep] Error in Huber regression for channel {ch}, time {t}: {e}"
                        )
        ...
        if "ransac" in methods:
            logging.info(
                "[BaselineCorrectionStep] Applying robust RANSAC regression..."
            )
            ...
            for ch in range(n_ch):
                ...
                for t in range(n_times):
                    ...
                    try:
                        ...
                        logging.warning(
                            f"[BaselineCorrectionStep] Error in RANSAC regression for channel {ch}, time {t}: {e}"
                        )
                        # Use standard regression as fallback
                        coef = LinearRegression().fit(X, y).coef_[0]
                        resid_D2[:, ch, t] = y - coef * X.ravel()
        ...
        if "kalman" in methods:
            logging.info(
                "[BaselineCorrectionStep] Applying Kalman filter for drift removal..."
            )
            ...
            if hasattr(epochs, "raw") and epochs.raw is not None:
                raw = epochs.raw.copy()
                logging.info(
                    "[BaselineCorrectionStep] Using raw data attached to epochs."
                )

            # If no raw data, skip this method
            if raw is None:
                logging.warning(
                    "[BaselineCorrectionStep] Raw data not available for Kalman method. Skipping."
                )
                if "kalman" in methods:
                ...
                try:
                    ...
                    P = 1.0
                    Q = 1e-5
                    R = 1e-2  # Process and measurement noise
                    ...
                    logging.error(
                        f"[BaselineCorrectionStep] Error applying Kalman method: {e}"
                    )
                    if "kalman" in methods:
                        methods.remove("kalman")
        ...
        if "glmm" in methods:
            logging.info(
                "[BaselineCorrectionStep] Applying GLMM baseline correction via MixedLM…"
            )
            ...
            if "subject" not in epochs.metadata:
                logging.info(
                    "[BaselineCorrectionStep] No subject metadata found—assigning dummy group for GLMM."
                )
            ...
            for ch in range(n_ch):
                for t in range(n_times):
                    ...
                    try:
                        ...
                        try:
                            fit = model.fit(
                                reml=False, method="lbfgs", warn_convergence=False
                            )
                        except Exception:
                            # fallback to conjugate gradient
                            fit = model.fit(
                                reml=False, method="cg", warn_convergence=False
                            )

                        resid_H[:, ch, t] = fit.resid
                    except Exception as e:
                        logging.warning(
                            f"[GLMM] Channel {ch} time {t} failed both optimizers ({e}); fallback to OLS."
                        )
                        # OLS fallback
                        ols = LinearRegression().fit(df[["baseline"]], df["voltage"])
                        resid_H[:, ch, t] = df["voltage"] - ols.predict(
                            df[["baseline"]]
                        )
        ...
        if plot_comparison and len(methods) > 1:
            logging.info(
                "[BaselineCorrectionStep] Generating comparison plot of baseline correction methods..."
            )
            ...
            plt.legend()
            title = (
                f"{plot_title} ({ch_name})"
                if plot_title
                else f"Comparison of Baseline Correction Methods ({ch_name})"
            )
            plt.title(title)
            plt.xlabel("Time (s)")
            ...
            if save_plots:
                plot_filename = os.path.join(
                    plot_dir, "baseline_methods_comparison.png"
                )
                plt.savefig(plot_filename, dpi=300)
                logging.info(
                    f"[BaselineCorrectionStep] Saved comparison plot to {plot_filename}"
                )
            ...
            if save_plots:
                plot_filename = os.path.join(
                    plot_dir, "baseline_methods_difference.png"
                )
                plt.savefig(plot_filename, dpi=300)
                logging.info(
                    f"[BaselineCorrectionStep] Saved difference plot to {plot_filename}"
                )
        ...
        if interactive and original_backend:
            try:
                ...
                if (
                    ...
                    logging.info(
                        f"[BaselineCorrectionStep] Switching matplotlib backend back to {original_backend}"
                    )
                    matplotlib.use(original_backend)
                    plt.ioff()  # Turn off interactive mode
            except Exception as e:
                logging.warning(
                    f"[BaselineCorrectionStep] Error switching back to original backend: {e}"
                )

        # Return results
        if len(methods) == 1:
            logging.info(
                f"[BaselineCorrectionStep] Returning epochs corrected using {methods[0]} method"
            )
            return results[methods[0]]
        else:
            logging.info(
                f"[BaselineCorrectionStep] Returning dictionary of epochs corrected using multiple methods"
            )
            return results
