...
def _check_install_dependencies():
    ...
    if missing_deps:
        try:
            ...
            logging.info(
                f"[ICA] Installing missing dependencies: {', '.join(missing_deps)}"
            )
            ...
            if "mne-icalabel" in missing_deps:
                try:
                    ...
                    logging.warning(
                        "[ICA] Failed to import mne-icalabel even after installation"
                    )
            ...
            if "statsmodels" in missing_deps:
                try:
                    ...
                    logging.warning(
                        "[ICA] Failed to import statsmodels even after installation"
                    )

        except Exception as e:
            logging.warning(f"[ICA] Failed to install missing dependencies: {e}")
            logging.warning(
                "[ICA] Some advanced ICA visualizations may not be available"
            )
            logging.warning(
                f"[ICA] To manually install, run: pip install {' '.join(missing_deps)}"
            )
...
class ICAStep(BaseStep):
    ...
    def run(self, data):
        ...
        if params["interactive"]:
            try:
                ...
                if (
                    ...
                    logging.info(
                        f"[ICAStep] Switching matplotlib backend from {original_backend} to TkAgg for interactive plotting"
                    )
                ...
                logging.warning(
                    f"[ICAStep] Could not switch to interactive backend: {e}"
                )
        ...
        if n_components == "all":
            logging.info(
                "[ICAStep] Using all available components for ICA decomposition."
            )
        ...
        if params["use_good_epochs_only"]:
            logging.info(
                "[ICAStep] Using data with AutoReject annotations to exclude bad segments."
            )
            ...
            if data.annotations is not None and len(data.annotations) > 0:
                ...
                if bad_annot_count > 0:
                    ...
                    logging.info(
                        f"[ICAStep] Found {bad_annot_count} 'BAD_autoreject' annotations"
                    )
            ...
            if has_autoreject_annotations:
                ...
                logging.info(
                    f"[ICAStep] Created {len(good_epochs)} epochs, excluding segments with bad annotations"
                )
            else:
                # No annotations found, use all data
                logging.warning(
                    "[ICAStep] No BAD_autoreject annotations found. Using all data for ICA."
                )
                events = mne.make_fixed_length_events(data, duration=1)
                good_epochs = mne.Epochs(
                ...
                logging.info(
                    f"[ICAStep] Created {len(good_epochs)} epochs (no annotations to exclude)"
                )
        else:
            logging.info(
                "[ICAStep] Using all data for ICA (use_good_epochs_only=False)."
            )
            good_epochs = data
        ...
        if bad_ic_candidates:
            ...
            for entry in bad_ic_candidates:
                print(
                    f"  {entry['type']} => {entry['indices']} (threshold={entry['threshold']})"
                )
            print(
                "These are NOT excluded yet. You will get a chance to confirm or modify.\n"
            )
        ...
        if params["interactive"]:
            ...
            if params["plot_components"]:
                logging.info(
                    "[ICAStep] Opening plot of all ICA component topographies..."
                )
                print(
                    "[ICAStep] Opening component topographies. Close the window when you're done reviewing."
                )

                try:
                    # Try interactive plotting first
                    fig_topo = ica.plot_components(
                        picks=range(min(ica.n_components_, 20)), show=True
                    )
                    input(
                        "[ICAStep] Press Enter after closing the component topography window to continue..."
                    )
                except Exception as e:
                    # Fallback: save plots to files
                    logging.warning(f"[ICAStep] Interactive plotting failed: {e}")
                    logging.info(
                        "[ICAStep] Saving component topographies to files instead."
                    )
                    figs = ica.plot_components(
                        picks=range(min(ica.n_components_, 20)), show=False
                    )
                    ...
                    print(
                        "[ICAStep] Please review the component plots and be ready to enter component numbers to exclude."
                    )
            ...
            if params["plot_sources"]:
                ...
                print(
                    "[ICAStep] Opening source time courses. Close the window when you're done reviewing."
                )
                ...
                try:
                    ...
                    input(
                        "[ICAStep] Press Enter after closing the source time courses window to continue..."
                    )
                except Exception as e:
                    # Fallback: save plot to file
                    logging.warning(
                        f"[ICAStep] Interactive plotting of sources failed: {e}"
                    )
                    logging.info(
                        "[ICAStep] Saving source time courses to file instead."
                    )
            ...
            if union_candidates:
                logging.info(
                    "[ICAStep] Opening detailed properties of candidate bad components..."
                )
                print(
                    "[ICAStep] Opening detailed properties of candidate bad components. Close each window when done."
                )

                try:
                    # Try interactive plotting first
                    ica.plot_properties(
                        good_epochs, picks=sorted(list(union_candidates)), show=True
                    )
                    input(
                        "[ICAStep] Press Enter after reviewing component properties to continue..."
                    )
                except Exception as e:
                    # Fallback: save property plots to files
                    logging.warning(
                        f"[ICAStep] Interactive plotting of properties failed: {e}"
                    )
                    logging.info(
                        "[ICAStep] Saving component properties to files instead."
                    )

                    for comp_idx in sorted(list(union_candidates)):
                        try:
                            fig = ica.plot_properties(
                                good_epochs, picks=comp_idx, show=False
                            )
                            fig_path = os.path.join(
                                plot_dir, f"ica_properties_comp_{comp_idx}.png"
                            )
                            ...
                            logging.warning(
                                f"[ICAStep] Error saving properties for component {comp_idx}: {prop_e}"
                            )
            ...
            if final_exclude:
                logging.info(
                    "[ICAStep] Opening detailed properties of pre-excluded components..."
                )
                print(
                    "[ICAStep] Opening detailed properties of pre-excluded components. Close each window when done."
                )

                try:
                    # Try interactive plotting first
                    ica.plot_properties(
                        good_epochs, picks=sorted(list(final_exclude)), show=True
                    )
                    input(
                        "[ICAStep] Press Enter after reviewing pre-excluded component properties to continue..."
                    )
                except Exception as e:
                    # Fallback: save property plots to files
                    logging.warning(
                        f"[ICAStep] Interactive plotting of pre-excluded properties failed: {e}"
                    )
                    logging.info(
                        "[ICAStep] Saving pre-excluded component properties to files instead."
                    )
                    ...
                    for comp_idx in sorted(list(final_exclude)):
                        try:
                            ...
                            fig_path = os.path.join(
                                plot_dir, f"ica_preexcluded_comp_{comp_idx}.png"
                            )
                            fig[0].savefig(fig_path)
                            for f in fig:
                                plt.close(f)
                            ...
                            logging.warning(
                                f"[ICAStep] Error saving properties for pre-excluded component {comp_idx}: {prop_e}"
                            )

                    print(
                        f"[ICAStep] Pre-excluded component properties saved to {plot_dir}"
                    )
            ...
            print(
                "Components with muscle artifacts typically show high-frequency activity."
            )
            print(
                "Components with cardiac artifacts typically show regular periodic activity."
            )
            ...
            if user_input.strip():
                ...
                try:
                    ...
                    logging.error(
                        "[ICAStep] Invalid input format. Please enter comma-separated numbers only."
                    )
                    print("Invalid input format. Using suggested components instead.")
                    final_exclude = union_candidates
            else:
                print(
                    "No components manually selected. Using only automatically detected components."
                )
                final_exclude = union_candidates
        else:
            logging.info(
                "[ICAStep] Non-interactive mode. Using only 'exclude' param from YAML."
            )
        ...
        if params["interactive"] and original_backend not in [
            ...
            try:
                logging.info(
                    f"[ICAStep] Switching matplotlib backend back to {original_backend}"
                )
                matplotlib.use(original_backend)
                plt.ioff()  # Turn off interactive mode
            except Exception as e:
                logging.warning(
                    f"[ICAStep] Error switching back to original backend: {e}"
                )
        ...
        ica_dir = (
            paths.get_derivative_path(sub_id, ses_id)
        ...
        try:
            ...
            alt_dir = Path(
                os.path.join(
                    str(paths.get_derivative_path(sub_id, ses_id)),
                    f"sub-{sub_id}_ses-{ses_id}_desc-ica_cleaned.fif",
                )
            )
            try:
                ...
                logging.info(
                    f"[ICAStep] Saved ICA-cleaned data to alternative path: {alt_dir}"
                )
            except Exception as e2:
                logging.error(
                    f"[ICAStep] Error saving to alternative path as well: {e2}"
                )
    ...
    def _generate_report(self, ica, data, params, paths):
        ...
        try:
            ...
            if importlib.util.find_spec("mne_icalabel") is None:
                logging.warning(
                    "[ICAStep] mne-icalabel package is not installed. Advanced classification will not be available."
                )
                ...
                logging.info(
                    f"[ICAStep] ICLabel classification successful: {ic_labels['labels']}"
                )
                has_iclabel = True
                ...
                for label in ["eye", "heart", "muscle", "line_noise"]:
                    indices = [
                        idx for idx, l in enumerate(ic_labels["labels"]) if l == label
                    ]
                    if indices:
        ...
        try:
            ...
            ax_overview.set_title(
                "ICA Components Overview", fontsize=14, fontweight="bold"
            )
            ...
            for i in range(n_components):
                ...
                for artifact_type, indices in artifact_labels.items():
                    if (
                        i in indices and artifact_type != "Excluded"
                    ):  # Skip 'Excluded' to avoid double coloring
            ...
            for label, color in colors.items():
                if label == "Good" or any(
                    label.lower() in k.lower() for k in artifact_labels.keys()
                ):
            ...
            if has_iclabel:
                ...
                for i in range(n_components):
                    ax_iclabel.text(
                        i, -0.05, str(i), ha="center", va="center", fontsize=8
                    )
            ...
            try:
                ica.plot_components(
                    picks=range(min(20, n_components)),
                    ch_type="eeg",
                    axes=ax_topo,
                    title="Component Topographies",
                    colorbar=False,
                    show=False,
                )
            except Exception as e:
                ...
                ax_topo.text(
                    0.5,
                    0.5,
                    "Component topographies could not be plotted",
                    ha="center",
                    va="center",
                    fontsize=12,
                )
        ...
        try:
            ...
            for comp_idx in range(n_components):
                ...
                plt.suptitle(
                    f"Component {comp_idx} Analysis", fontsize=16, fontweight="bold"
                )
                ...
                if has_iclabel:
                    ...
                    prob = ic_labels["y_pred_proba"][comp_idx][
                        ic_labels["labels_set"].index(label)
                    ]
                    classification_text += (
                        f"ICLabel: {label.capitalize()} ({prob:.2f})\n"
                    )
                ...
                try:
                    ...
                    logging.warning(
                        f"[ICAStep] Error plotting component topography: {e}"
                    )
                    axes[0, 0].text(
                        0.5, 0.5, "Topography plot failed", ha="center", va="center"
                    )
                ...
                try:
                    ...
                    f, Pxx = signal.welch(
                        source, fs=sfreq, nperseg=min(4096, len(source))
                    )
                    ...
                    axes[0, 1].set_xlim(
                        [0, min(100, sfreq / 2)]
                    )  # Limit to 100 Hz or Nyquist
                    axes[0, 1].grid(True)
                except Exception as e:
                    logging.warning(f"[ICAStep] Error plotting component PSD: {e}")
                    axes[0, 1].text(
                        0.5, 0.5, "PSD plot failed", ha="center", va="center"
                    )
                ...
                try:
                    ...
                    logging.warning(
                        f"[ICAStep] Error plotting component time series: {e}"
                    )
                    axes[1, 0].text(
                        0.5, 0.5, "Time series plot failed", ha="center", va="center"
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        axes[1, 1].hist(
                            source, bins=50, density=True, alpha=0.8, color="steelblue"
                        )
                        ...
                        skew = (
                            np.mean(((source - mean) / std_dev) ** 3)
                            if std_dev > 0
                            else 0
                        )
                        kurtosis = (
                            np.mean(((source - mean) / std_dev) ** 4) - 3
                            if std_dev > 0
                            else 0
                        )
                        ...
                        logging.warning(
                            f"[ICAStep] Error creating custom component properties: {e1}"
                        )
                        ...
                        try:
                            ...
                            logging.warning(
                                f"[ICAStep] Error plotting autocorrelation: {e2}"
                            )
                            axes[1, 1].text(
                                0.5,
                                0.5,
                                "Properties plot failed",
                                ha="center",
                                va="center",
                            )
                except Exception as e:
                    ...
                    axes[1, 1].text(
                        0.5, 0.5, "Properties section failed", ha="center", va="center"
                    )
                ...
                try:
                    ...
                    logging.error(
                        f"[ICAStep] Error saving component plot for component {comp_idx}: {e}"
                    )
                plt.close(fig)

            logging.info(
                f"[ICAStep] Saved detailed plots for {component_files_created} components to {comp_dir}"
            )
        except Exception as e:
        ...
        try:
            ...
            try:
                with open(str(readme_file), "w", encoding="utf-8") as f:
                    ...
                    f.write(f"**Subject:** {sub_id}, **Session:** {ses_id}\n\n")
                    f.write(
                        "Error occurred during full README generation. See console logs for details.\n"
                    )
                logging.info(
                    f"[ICAStep] Saved minimal README to {readme_file} after error"
                )
            except Exception as e2:
