...
class AutoRejectStep(BaseStep):
    ...
    def run(self, data):
        ...
        logging.info(
            f"[AutoRejectStep] Is instance of mne.Epochs: {isinstance(data, mne.Epochs)}"
        )

        if hasattr(mne, "EpochsFIF"):
            logging.info(
                f"[AutoRejectStep] Is instance of mne.EpochsFIF: {isinstance(data, mne.EpochsFIF)}"
            )
        ...
        if interactive:
            ...
            try:
                ...
                if (
                    ...
                    logging.info(
                        f"[AutoRejectStep] Switching matplotlib backend from {original_backend} to TkAgg for interactive plotting"
                    )
                    matplotlib.use("TkAgg")
                    plt.ion()  # Turn on interactive mode
            except Exception as e:
                logging.warning(
                    f"[AutoRejectStep] Could not switch to interactive backend: {e}"
                )
                logging.warning(
                    "[AutoRejectStep] Interactive plots may not work properly"
                )
        ...
        if (
            ...
            epochs_tmp = mne.Epochs(
                data, events_tmp, tmin=0, tmax=1, baseline=None, detrend=0, preload=True
            )
        ...
        if mode == "fit":
            logging.info(
                "[AutoRejectStep] Running in 'fit' mode - only identifying bad epochs"
            )
            ...
            if already_epoched:
                ...
                logging.info(
                    "[AutoRejectStep] Data already in epochs format. Bad epochs stored in data.info['temp']['autoreject_bad_epochs']"
                )
                logging.info(
                    f"[AutoRejectStep] Found {len(bad_epoch_indices)} bad epochs out of {len(reject_log.bad_epochs)}"
                )
            else:
                # Convert reject log to annotations and apply to raw data
                cleaned_data = data.copy()
                self._add_reject_log_as_annotations(
                    cleaned_data, reject_log, events_tmp
                )
        else:  # fit_transform mode
            logging.info(
                "[AutoRejectStep] Running in 'fit_transform' mode - cleaning the data"
            )
            ...
            if not already_epoched and self.params.get("return_raw", True):
                logging.info(
                    "[AutoRejectStep] Converting cleaned epochs back to raw data."
                )
        ...
        logging.info(
            f"[AutoRejectStep] AutoReject identified {n_bad_epochs} bad epochs out of {n_epochs_before} ({n_rejected_percent:.1f}%)"
        )

        # Generate simple visualization if requested
        if plot_results:
            self._create_simple_visualization(
                reject_log, data, ar, plot_dir, interactive
            )
        ...
        if "temp" in cleaned_data.info:
            if "autoreject_bad_epochs" in cleaned_data.info["temp"]:
                logging.info(
                    f"[AutoRejectStep] VERIFICATION: Found {len(cleaned_data.info['temp']['autoreject_bad_epochs'])} bad epochs in info['temp']"
                )
            else:
                logging.warning(
                    "[AutoRejectStep] VERIFICATION: 'autoreject_bad_epochs' key not found in info['temp']"
                )
        else:
            logging.warning(
                "[AutoRejectStep] VERIFICATION: 'temp' dictionary not found in info"
            )

        # Save the AutoReject model state if requested
        if save_model:
            self._save_model_state(
                ar, model_filename, output_dir
            )  # TODO: we need to find anotehr way to save it and load it when we need the state and log
        ...
        if interactive and original_backend:
            try:
                if (
                    ...
                    logging.info(
                        f"[AutoRejectStep] Switching matplotlib backend back to {original_backend}"
                    )
                    matplotlib.use(original_backend)
                    plt.ioff()  # Turn off interactive mode
            except Exception as e:
                logging.warning(
                    f"[AutoRejectStep] Error switching back to original backend: {e}"
                )
    ...
    def _add_reject_log_as_annotations(self, raw, reject_log, events):
        ...
        if events is None:
            logging.info(
                "[AutoRejectStep] Cannot add annotations because no events were provided (data was already epoched)."
            )
            return
        ...
        if not any(bad_epochs):
            logging.info(
                "[AutoRejectStep] No bad epochs detected, no annotations added"
            )
            return
    ...
    def _create_simple_visualization(
        self, reject_log, raw, ar=None, plot_dir=None, interactive=False
    ):
        """
        ...
        try:
            ...
            if plot_dir is None:
                ...
                plot_dir = (
                    f"./data/processed/sub-{sub_id}/ses-{ses_id}/figures/autoreject"
                )
            ...
            plt.title(
                f"AutoReject Results: {bad_epoch_percent:.1f}% of epochs marked bad"
            )

            # Create bar plot showing bad epochs (1=bad, 0=good)
            plt.bar(
                range(n_epochs),
                [1 if x else 0 for x in bad_epochs],
                color="r",
                alpha=0.7,
            )
            ...
            plt.savefig(
                os.path.join(
                    plot_dir, f"autoreject_{sub_id}_{ses_id}_run-{run_id}_epochs.png"
                ),
                dpi=150,
            )
            ...
            if ar is not None and hasattr(ar, "threshes_"):
                try:
                    ...
                    if ar.threshes_ is None:
                        ...
                        logging.warning(
                            "[AutoRejectStep] No thresholds available in AutoReject object"
                        )
                        plt.close()
                        return
                    ...
                    for ch_idx, ch_name in enumerate(ch_names):
                        try:
                            if (
                                ch_idx < len(ar.threshes_)
                                and ar.threshes_[ch_idx] is not None
                            ):
                            ...
                            logging.warning(
                                f"[AutoRejectStep] Error accessing threshold for channel {ch_name}: {e}"
                            )
                    ...
                    if not any(ch_thresholds):
                        logging.warning(
                            "[AutoRejectStep] No valid thresholds found for visualization"
                        )
                        plt.close()
                        return
                    ...
                    plt.savefig(
                        os.path.join(
                            plot_dir,
                            f"autoreject_{sub_id}_{ses_id}_run-{run_id}_thresholds.png",
                        ),
                        dpi=150,
                    )
                    ...
                    logging.warning(
                        f"[AutoRejectStep] Error creating thresholds visualization: {e}"
                    )
    ...
    def _plot_channel_rejection_rates(
        self, reject_log, raw, plot_dir=None, interactive=False
    ):
        """
        ...
        try:
            ...
            ch_types = (
                raw.get_channel_types() if hasattr(raw, "get_channel_types") else []
            )
            excluded_channels = []
            ...
            if ch_types:
                for idx, (ch_name, ch_type) in enumerate(zip(ch_names, ch_types)):
                    if ch_type == "stim" or any(
                        keyword in ch_name.lower() for keyword in trigger_keywords
                    ):
            ...
            if excluded_channels:
                logging.info(
                    f"[AutoRejectStep] Excluding trigger/stim channels from rejection plot: {excluded_channels}"
                )
            ...
            if (
                not ch_rejection_rates
                and hasattr(reject_log, "threshes")
                and reject_log.threshes is not None
            ):
                for ch_idx, ch_name in enumerate(ch_names):
                    # Skip excluded channels
                    if ch_name in excluded_channels:
                    ...
                    if (
                        ch_idx < len(reject_log.threshes)
                        and reject_log.threshes[ch_idx] is not None
                    ):
                        # Normalize thresholds to a 0-100 scale for visualization
                        max_thresh = np.max(
                            [t for t in reject_log.threshes if t is not None]
                        )
                        norm_thresh = (
                            (reject_log.threshes[ch_idx] / max_thresh) * 100
                            if max_thresh > 0
                            else 0
                        )
            ...
            if (
                not ch_rejection_rates
                and hasattr(reject_log, "scores")
                and reject_log.scores is not None
            ):
                for ch_idx, ch_name in enumerate(ch_names):
                    # Skip excluded channels
                    if ch_name in excluded_channels:
                    ...
                    if ch_idx < reject_log.scores.shape[1]:
                        ...
                        mean_score = (
                            np.mean(reject_log.scores[:, ch_idx]) * 100
                        )  # Scale to percentage
            ...
            if not ch_rejection_rates:
                logging.warning(
                    "[AutoRejectStep] No channel-wise rejection data available for visualization"
                )
                return

            # Sort channels by rejection rate (highest to lowest)
            sorted_channels = sorted(
                ch_rejection_rates.items(), key=lambda x: x[1], reverse=True
            )

            # Create figure
            plt.figure(
                figsize=(12, max(8, len(ch_rejection_rates) / 3))
            )  # Adjust height based on channel count
            ...
            plt.savefig(
                os.path.join(
                    plot_dir,
                    f"autoreject_{sub_id}_{ses_id}_run-{run_id}_channel_rejection_rates.png",
                ),
                dpi=150,
            )
            ...
            logging.info(
                f"[AutoRejectStep] Saved channel rejection rates visualization to {plot_dir}"
            )

        except Exception as e:
            logging.warning(
                f"[AutoRejectStep] Error creating channel rejection rates visualization: {e}"
            )
            import traceback
    ...
    def _save_cleaned_data(self, data, file_prefix, output_dir):
        ...
        try:
            ...
            if output_dir is None:
                if (
                    hasattr(self.params, "paths")
                    and self.params.get("paths") is not None
                ):
                    ...
                    if hasattr(paths, "get_checkpoint_path"):
                        ...
                        base_dir = os.path.join(
                            f"./data/processed/sub-{sub_id}/ses-{ses_id}"
                        )
                        os.makedirs(base_dir, exist_ok=True)
                        path = os.path.join(
                            base_dir,
                            f"sub-{sub_id}_ses-{ses_id}_run-{run_id}_{file_prefix}_epo.fif",
                        )
                else:
            ...
            for key in data_to_save.info.keys():
                if key in ["description", "experimenter", "proj_name", "subject_info"]:
                    if isinstance(data_to_save.info[key], dict):
                        ...
                        hasattr(data_to_save.info[key], "dtype")
                        and data_to_save.info[key].dtype.kind == "S"
                    ):
                        data_to_save.info[key] = str(data_to_save.info[key])
            ...
            try:
                ...
                # Make sure subject_info exists and has his_id
                if (
                    not hasattr(data_copy.info, "subject_info")
                    or data_copy.info["subject_info"] is None
                ):
                ...
                logging.info(
                    f"[AutoRejectStep] Saved cleaned data to alternative path: {alt_path}"
                )
            except Exception as alt_e:
                logging.error(
                    f"[AutoRejectStep] Error saving to alternative path: {alt_e}"
                )
    ...
    def _store_reject_log_in_info(self, data, reject_log, epochs, ar):
        ...
        try:
            ...
            logging.info(
                f"[AutoRejectStep] Stored AutoReject information in data.info['temp']"
            )
            logging.info(
                f"[AutoRejectStep] Bad epochs: {len(bad_indices)} out of {len(reject_log.bad_epochs)}"
            )

        except Exception as e:
            logging.error(
                f"[AutoRejectStep] Error storing reject_log in data.info: {e}"
            )
    ...
    def _save_model_state(self, ar, model_filename, output_dir):
        ...
        try:
            ...
            try:
                ...
                h5io_available = False
                logging.warning(
                    "[AutoRejectStep] h5io module not available. Will use pickle instead."
                )
            ...
            if h5io_available and model_filename.endswith(".h5"):
                ...
                try:
                    write_hdf5(
                        file_path, ar.__getstate__(), title="autoreject", overwrite=True
                    )
                    logging.info(
                        f"[AutoRejectStep] Saved AutoReject model state to {file_path}"
                    )
                    return
                except Exception as e:
                    logging.warning(
                        f"[AutoRejectStep] Error saving model with HDF5: {e}"
                    )
            ...
            with open(pickle_path, "wb") as f:
                pickle.dump(ar, f)
            logging.info(
                f"[AutoRejectStep] Saved AutoReject model to {pickle_path} using pickle"
            )
    ...
    def load_model(model_path):
        ...
        if model_path.endswith(".h5"):
            ...
            raise ValueError(
                f"Unsupported file format: {model_path}. Expected .h5 or .pkl"
            )
