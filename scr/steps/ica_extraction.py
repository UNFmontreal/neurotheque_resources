...
def _check_install_dependencies():
    ...
    if missing_deps:
        try:
            ...
            logging.info(
                f"[ICAExtractionStep] Installing missing dependencies: {', '.join(missing_deps)}"
            )
            ...
            if "mne-icalabel" in missing_deps:
                try:
                    ...
                    logging.warning(
                        "[ICAExtractionStep] Failed to import mne-icalabel even after installation"
                    )
            ...
            if "statsmodels" in missing_deps:
                try:
                    ...
                    logging.warning(
                        "[ICAExtractionStep] Failed to import statsmodels even after installation"
                    )

        except Exception as e:
            logging.warning(
                f"[ICAExtractionStep] Failed to install missing dependencies: {e}"
            )
            logging.warning(
                "[ICAExtractionStep] Some advanced ICA visualizations may not be available"
            )
            logging.warning(
                f"[ICAExtractionStep] To manually install, run: pip install {' '.join(missing_deps)}"
            )
...
class ICAExtractionStep(BaseStep):
    ...
    def run(self, data):
        ...
        if not params["interactive"] and "ipykernel" in sys.modules:
            try:
                logging.info(
                    "[ICAExtractionStep] Jupyter environment detected, using inline backend"
                )
                matplotlib.use("inline")
            except Exception as e:
                logging.warning(
                    f"[ICAExtractionStep] Could not switch to inline backend: {e}"
                )
        # Check if we need to switch backends for interactive plotting
            ...
            try:
                ...
                if (
                    original_backend != "TkAgg"
                    and original_backend != "Qt5Agg"
                    and original_backend != "WXAgg"
                ):
                    logging.info(
                        f"[ICAExtractionStep] Switching matplotlib backend from {original_backend} to TkAgg for interactive plotting"
                    )
                    try:
                        ...
                        try:
                            logging.info(
                                "[ICAExtractionStep] TkAgg failed, trying Qt5Agg"
                            )
                            matplotlib.use("Qt5Agg")
                        except Exception:
                            # If Qt5Agg fails, fall back to Agg
                            logging.warning(
                                "[ICAExtractionStep] All interactive backends failed, using Agg"
                            )
                            matplotlib.use("Agg")
                ...
                logging.warning(
                    f"[ICAExtractionStep] Could not switch to interactive backend: {e}"
                )
                logging.warning(
                    "[ICAExtractionStep] Interactive plots may not work properly"
                )
        ...
        if n_components == "all":
            logging.info(
                "[ICAExtractionStep] Using all available components for ICA decomposition."
            )
        ...
        if params["use_good_epochs_only"]:
            logging.info(
                "[ICAExtractionStep] Using data with AutoReject annotations to exclude bad segments."
            )
            ...
            if data.annotations is not None and len(data.annotations) > 0:
                bad_annot_count = sum(
                    1
                    for desc in data.annotations.description
                    if desc == "BAD_autoreject"
                )
                if bad_annot_count > 0:
                    has_autoreject_annotations = True
                    logging.info(
                        f"[ICAExtractionStep] Found {bad_annot_count} 'BAD_autoreject' annotations"
                    )
            ...
            if has_autoreject_annotations:
                ...
                logging.info(
                    f"[ICAExtractionStep] Created {len(good_epochs)} epochs, excluding segments with bad annotations"
                )

            elif (
                isinstance(data, mne.Epochs)
                or str(type(data).__name__) == "EpochsFIF"
                or hasattr(data, "epochs")
            ):
                ...
                if "temp" in data.info:
                    if "autoreject_bad_epochs" in data.info["temp"]:
                        ...
                        logging.info(
                            f"[ICAExtractionStep] Found {len(bad_indices)} bad epochs from autoreject_bad_epochs"
                        )
                    elif (
                        "autoreject" in data.info["temp"]
                        and "bad_epochs" in data.info["temp"]["autoreject"]
                    ):
                        # Method 2: Boolean array from full autoreject info
                        bad_epochs_bool = data.info["temp"]["autoreject"]["bad_epochs"]
                        bad_indices = [
                            i for i, bad in enumerate(bad_epochs_bool) if bad
                        ]
                        logging.info(
                            f"[ICAExtractionStep] Found {len(bad_indices)} bad epochs from autoreject info"
                        )
                ...
                if bad_indices:
                    ...
                    logging.info(
                        f"[ICAExtractionStep] Using {len(good_epochs)}/{len(data)} good epochs for ICA"
                    )
                else:
                    # No bad epochs found, use all epochs
                    logging.warning(
                        "[ICAExtractionStep] No autoreject information found. Using all epochs for ICA."
                    )
                    good_epochs = data
                    logging.info(
                        f"[ICAExtractionStep] Using all {len(good_epochs)} epochs for ICA"
                    )
            else:
                # No annotations found, use all data
                logging.warning(
                    "[ICAExtractionStep] No BAD_autoreject annotations found. Using all data for ICA."
                )
                events = mne.make_fixed_length_events(data, duration=1)
                good_epochs = mne.Epochs(
                    data, events, tmin=0, tmax=1, baseline=None, preload=True
                )
                logging.info(
                    f"[ICAExtractionStep] Created {len(good_epochs)} epochs (no annotations to exclude)"
                )
        else:
            logging.info(
                "[ICAExtractionStep] Using all data for ICA (use_good_epochs_only=False)."
            )
            good_epochs = data
        ...
        # --------------------------
        logging.info(
            f"[ICAExtractionStep] Fitting ICA with {ica.n_components} components using {params['method']} method..."
        )
        try:
            ica.fit(
                good_epochs,
        ...
        if params["plot_properties"]:
            prop_figs = self._plot_component_properties(
                ica, good_epochs, plot_dir, params
            )
            if prop_figs:
        ...
        if params.get("plot_details", False):
            try:
                self._generate_component_details(
                    ica, data, good_epochs, plot_dir, params
                )
            except Exception as e:
                logging.error(
                    f"[ICAExtractionStep] Error generating component details: {e}"
                )
                logging.error(
                    "[ICAExtractionStep] Continuing without component details"
                )

        # --------------------------
        # 6) Save ICA solution
        # --------------------------
        if paths is not None and hasattr(
            ica, "n_components_"
        ):  # Only save if ICA was successfully fitted
            try:
                # Use normalized path handling for Windows compatibility
                ica_dir = os.path.normpath(
                    os.path.dirname(str(paths.get_derivative_path(sub_id, ses_id)))
                )
                ...
                logging.info(
                    f"[ICAExtractionStep] Saving ICA decomposition to {ica_file}"
                )
                ica_clean.save(ica_file, overwrite=True)
            except Exception as e:
                logging.error(
                    f"[ICAExtractionStep] Error saving ICA decomposition: {e}"
                )
                logging.error(
                    "[ICAExtractionStep] Continuing without saving ICA decomposition"
                )

        # Save the data with ICA components if requested
        save_data = params.get("save_data", False)
        if save_data and hasattr(
            ica, "n_components_"
        ):  # Only save if ICA was successfully fitted
            try:
                if paths is None:
                    logging.error(
                        "[ICAExtractionStep] Cannot save data: paths object is None"
                    )
                    return data
                ...
                file_name = (
                    f"sub-{sub_id}_ses-{ses_id}{task_str}{run_str}_desc-ica_epo.fif"
                )
                ...
                # Save the data
                logging.info(
                    f"[ICAExtractionStep] Saving data with ICA components to {file_path}"
                )
                try:
                    ...
                    logging.error(
                        f"[ICAExtractionStep] Error in data save operation: {save_err}"
                    )
            except Exception as e:
                logging.error(
                    f"[ICAExtractionStep] Error saving data with ICA components: {e}"
                )
        ...
        if params["interactive"] and original_backend not in [
            "TkAgg",
            "Qt5Agg",
            "WXAgg",
        ]:
            try:
                logging.info(
                    f"[ICAExtractionStep] Switching matplotlib backend back to {original_backend}"
                )
                matplotlib.use(original_backend)
                plt.ioff()  # Turn off interactive mode
            except Exception as e:
                logging.warning(
                    f"[ICAExtractionStep] Error switching back to original backend: {e}"
                )
    ...
    def _plot_component_topographies(self, ica, plot_dir, params):
        ...
        try:
            ...
            if hasattr(ica, "n_components_"):
                ...
                n_components = (
                    ica.mixing_matrix_.shape[1]
                    if hasattr(ica, "mixing_matrix_")
                    else 20
                )
                logging.warning(
                    f"[ICAExtractionStep] Could not determine n_components, using: {n_components}"
                )
            ...
            for start_idx in range(0, n_components, n_per_plot):
                ...
                if isinstance(fig, list):
                    for i, f in enumerate(fig):
                        f.savefig(
                            os.path.join(topo_dir, f"components_{start_idx+i:03d}.png"),
                            dpi=300,
                        )
                else:
                    fig.savefig(
                        os.path.join(
                            topo_dir, f"components_{start_idx:03d}-{end_idx-1:03d}.png"
                        ),
                        dpi=300,
                    )
            ...
            logging.error(
                f"[ICAExtractionStep] Error in _plot_component_topographies: {e}"
            )
    ...
    def _plot_source_timecourses(self, ica, data, plot_dir, params):
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    f"[ICAExtractionStep] Error saving source time course plot: {e}"
                )
            ...
            try:
                ...
                if (
                    isinstance(sources, mne.Epochs)
                    or str(type(sources).__name__) == "EpochsFIF"
                ):
                    # For epochs, take the first epoch or average across epochs
                    source_data = sources.get_data().mean(
                        axis=0
                    ...
                    for comp_idx in range(ica.n_components_):
                        try:
                            ...
                            fig.tight_layout()
                            fig.savefig(
                                os.path.join(sources_dir, f"source_{comp_idx:03d}.png"),
                                dpi=300,
                            )
                            all_figs.append(fig)
                        except Exception as e:
                            logging.error(
                                f"[ICAExtractionStep] Error plotting individual source {comp_idx}: {e}"
                            )
                    ...
                    logging.warning(
                        f"[ICAExtractionStep] Unsupported data type for individual source plots: {type(sources)}"
                    )

            except Exception as e:
                logging.error(
                    f"[ICAExtractionStep] Error creating individual source plots: {e}"
                )
    ...
    def _plot_component_properties(self, ica, epochs, plot_dir, params):
        ...
        try:
            ...
            for comp_idx in range(ica.n_components_):
                try:
                    logging.info(
                        f"[ICAExtractionStep] Plotting properties for component {comp_idx}"
                    )
                    ...
                    for i, fig in enumerate(figs):
                        fig.savefig(
                            os.path.join(
                                props_dir, f"comp_{comp_idx:03d}_property_{i}.png"
                            ),
                            dpi=300,
                        )
                        all_figs.append(fig)
                    ...
                    logging.error(
                        f"[ICAExtractionStep] Error processing properties for component {comp_idx}: {e}"
                    )

        except Exception as e:
            logging.error(
                f"[ICAExtractionStep] Error in _plot_component_properties: {e}"
            )
    ...
    def _plot_component_psd(self, ica, data, plot_dir, params):
        ...
        try:
            ...
            if (
                ...
                source_data_array = sources.get_data().mean(
                    axis=0
                )  # Average across epochs
            elif isinstance(sources, mne.io.Raw):
                # For Raw data
                ...
                logging.warning(
                    f"[ICAExtractionStep] Unsupported data type for PSD plots: {type(sources)}"
                )
                return all_figs
            ...
            try:
                ...
                for comp_idx in range(ica.n_components_):
                    ...
                    f, Pxx = signal.welch(
                        source_data, fs=sfreq, nperseg=min(4096, len(source_data))
                    )
                    ax.semilogy(
                        f,
                        Pxx,
                        alpha=0.7,
                        label=f"Comp {comp_idx}" if comp_idx < 10 else None,
                    )
                ...
                ):  # Only show legend if not too many components
                ...
                logging.error(
                    f"[ICAExtractionStep] Error creating combined PSD plot: {e}"
                )
            ...
            for comp_idx in range(ica.n_components_):
                try:
                    ...
                    fig.tight_layout()
                    fig.savefig(
                        os.path.join(psd_dir, f"comp_{comp_idx:03d}_psd.png"), dpi=300
                    )
                    all_figs.append(fig)
                except Exception as e:
                    logging.error(
                        f"[ICAExtractionStep] Error creating PSD plot for component {comp_idx}: {e}"
                    )
        ...
        try:
            ...
            for comp_idx in range(ica.n_components_):
                try:
                    ...
                    axs[0].set_title(
                        f"Original Data (Top 5 channels for Component {comp_idx})"
                    )
                    ...
                    fig.tight_layout()
                    fig.savefig(
                        os.path.join(overlay_dir, f"comp_{comp_idx:03d}_overlay.png"),
                        dpi=300,
                    )
                    plt.close(fig)
                    ...
                    logging.error(
                        f"[ICAExtractionStep] Error creating overlay plot for component {comp_idx}: {e}"
                    )
        ...
        try:
            ...
            for comp_idx in range(ica.n_components_):
                try:
                    ...
                    # Title
                    fig.suptitle(
                        f"Component {comp_idx} Detailed Analysis",
                        fontsize=16,
                        fontweight="bold",
                    )
                    ...
                    ica.plot_components(
                        picks=comp_idx, axes=ax_topo, show=False, colorbar=False
                    )
                    ...
                    f, Pxx = signal.welch(
                        source_data,
                        fs=data.info["sfreq"],
                        nperseg=min(4096, len(source_data)),
                    )
                    ...
                    times = (
                        np.arange(min(10000, sources.n_times)) / sources.info["sfreq"]
                    )
                    ...
                    try:
                        ...
                        freqs = np.logspace(
                            np.log10(1), np.log10(min(100, sfreq / 2.5)), 20
                        )
                        n_cycles = freqs / 2.0
                        ...
                        logging.warning(
                            f"[ICAExtractionStep] Error computing time-frequency plot: {tf_e}"
                        )
                        ax_tf.text(
                            0.5,
                            0.5,
                            "Time-frequency plot could not be generated",
                            ha="center",
                            va="center",
                            fontsize=12,
                        )
                    ...
                    ax_hist.hist(
                        source_data, bins=50, density=True, alpha=0.8, color="steelblue"
                    )
                    ...
                    skew = (
                        np.mean(((source_data - mean) / std_dev) ** 3)
                        if std_dev > 0
                        else 0
                    )
                    kurtosis = (
                        np.mean(((source_data - mean) / std_dev) ** 4) - 3
                        if std_dev > 0
                        else 0
                    )
                    ...
                    try:
                        ...
                        logging.warning(
                            f"[ICAExtractionStep] Error computing autocorrelation: {acf_e}"
                        )
                        ax_acf.text(
                            0.5,
                            0.5,
                            "Autocorrelation plot could not be generated",
                            ha="center",
                            va="center",
                            fontsize=12,
                        )
                    ...
                    fig.savefig(
                        os.path.join(
                            details_dir, f"component_{comp_idx:03d}_details.png"
                        ),
                        dpi=300,
                    )
                    plt.close(fig)
                    ...
                    logging.error(
                        f"[ICAExtractionStep] Error generating details for component {comp_idx}: {e}"
                    )

        except Exception as e:
            logging.error(
                f"[ICAExtractionStep] Error in _generate_component_details: {e}"
            )
