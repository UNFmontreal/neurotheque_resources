...
def _check_install_dependencies():
    ...
    if missing_deps:
        try:
            logging.info(
                f"[ICALabelingStep] Installing missing dependencies: {', '.join(missing_deps)}"
            )
            ...
            for dep in missing_deps:
                try:
                    ...
                    logging.warning(
                        f"[ICALabelingStep] Failed to import {dep} even after installation"
                    )
        except Exception as e:
            logging.warning(
                f"[ICALabelingStep] Failed to install missing dependencies: {e}"
            )
            logging.warning(
                "[ICALabelingStep] Some advanced ICA visualizations may not be available"
            )
            logging.warning(
                f"[ICALabelingStep] To manually install, run: pip install {' '.join(missing_deps)}"
            )
...
class ICALabelingStep(BaseStep):
    ...
    def run(self, data):
        ...
        if not params["interactive"] and "ipykernel" in sys.modules:
            try:
                logging.info(
                    "[ICALabelingStep] Jupyter environment detected, using inline backend"
                )
                matplotlib.use("inline")
            except Exception as e:
                logging.warning(
                    f"[ICALabelingStep] Could not switch to inline backend: {e}"
                )
                ...
                if original_backend not in ["TkAgg", "Qt5Agg", "WXAgg"]:
                    for backend in [
                        ...
                        try:
                            ...
                            print(
                                f"[ICALabelingStep] Switched matplotlib backend to {backend} for interactive plotting"
                            )
                            break
                        except Exception:
                            continue
                    if matplotlib.get_backend() == "Agg":
                        print(
                            "[ICALabelingStep] WARNING: Using non-interactive Agg backend"
                        )
                        print(
                            "[ICALabelingStep] Plots will be saved to disk but not displayed"
                        )
                    else:
                        plt.ion()
                    print(
                        f"[ICALabelingStep] Using matplotlib backend: {matplotlib.get_backend()}"
                    )
            except Exception as e:
                logging.warning(
                    f"[ICALabelingStep] Could not switch to interactive backend: {e}"
                )
        ...
        if "temp" in data.info and "ica" in data.info["temp"]:
            ...
            if paths is not None:
                ica_file = (
                    ...
                    / f"sub-{sub_id}_ses-{ses_id}_desc-ica_decomposition.fif"
                )
                if os.path.isfile(ica_file):
                    try:
                        ica = mne.preprocessing.read_ica(ica_file)
                        logging.info(
                            f"[ICALabelingStep] Loaded ICA from file: {ica_file}"
                        )
                    except Exception as e:
                        raise ValueError(
                            f"[ICALabelingStep] Failed to load ICA from file: {e}"
                        )
                else:
                    raise ValueError(
                        f"[ICALabelingStep] ICA file not found: {ica_file}"
                    )
            else:
                raise ValueError(
                    "[ICALabelingStep] No ICA found in data and no paths provided to load from file."
                )
        ...
        if paths is not None:
            ...
            plot_dir = params.get(
                "plot_dir", f"./ica_labeling_plots/sub-{sub_id}_ses-{ses_id}"
            )
            ...
            if ic_labels:
                ...
                for idx in range(len(ic_labels["labels"])):
                    ...
                    if label in thresholds_ic and label_prob >= thresholds_ic[label]:
                        ...
                        logging.info(
                            f"[ICALabelingStep] Component {idx} classified as '{label}' (prob={label_prob:.2f})"
                        )
        ...
        if "correlation" in params["methods"]:
            ...
            if corr_artifacts:
                ...
                if (
                    "eog" in corr_artifacts
                    and corr_artifacts["eog"]
                    and "eog_scores" in corr_artifacts
                ):
                    for idx, score in zip(
                        corr_artifacts["eog"], corr_artifacts["eog_scores"]
                    ):
                        if float(score) >= 0.8:
                            all_artifacts.add(idx)
                            logging.info(
                                f"[ICALabelingStep] Component {idx} identified as EOG with score {float(score):.2f}"
                            )
                if (
                    "ecg" in corr_artifacts
                    and corr_artifacts["ecg"]
                    and "ecg_scores" in corr_artifacts
                ):
                    for idx, score in zip(
                        corr_artifacts["ecg"], corr_artifacts["ecg_scores"]
                    ):
                        if score >= 0.8:
                            all_artifacts.add(idx)
                            logging.info(
                                f"[ICALabelingStep] Component {idx} identified as ECG with score {score:.2f}"
                            )

        # Plot labeled components.
        if params["plot_labeled"]:
            labeled_figs = self._plot_labeled_components(
                ica, data, labeled_components, all_artifacts, plot_dir, params
            )
            if labeled_figs:
        ...
        if params["manual_selection"] and params["interactive"]:
            ...
            if "iclabel" in labeled_components:
                print("  ICLabel:")
                for comp_idx, label in enumerate(
                    labeled_components["iclabel"]["labels"]
                ):
                    if label != "brain":
                        if comp_idx < len(
                            labeled_components["iclabel"]["y_pred_proba"]
                        ):
                            label_idx = labeled_components["iclabel"][
                                "labels_set"
                            ].index(label)
                            prob = labeled_components["iclabel"]["y_pred_proba"][
                                comp_idx
                            ][label_idx]
                            print(
                                f"    Component {comp_idx}: {label.capitalize()} (prob={prob:.2f})"
                            )
            ...
            if user_input.strip():
                try:
                    ...
                    logging.info(
                        f"[ICALabelingStep] User selected components to exclude: {sorted(list(final_exclude))}"
                    )
                except ValueError:
                    logging.error(
                        "[ICALabelingStep] Invalid input format. Using suggested components instead."
                    )
                    final_exclude = all_artifacts
            else:
                logging.info(
                    "[ICALabelingStep] Using automatically identified artifact components"
                )
                final_exclude = all_artifacts
        elif params["auto_exclude"]:
            final_exclude = all_artifacts
            logging.info(
                f"[ICALabelingStep] Automatically excluding components: {sorted(list(final_exclude))}"
            )
        else:
            logging.info(
                "[ICALabelingStep] No components will be excluded (manual_selection=False, auto_exclude=False)"
            )
        ...
        if params["reconstruct"] and final_exclude:
            ...
            if params["plot_before_after"]:
                ba_figs = self._plot_before_after(
                    data, data_clean, ica, plot_dir, params
                )
                if ba_figs:
            ...
            if params.get("save_data", False):
                try:
                    if "output_file" in params:
                        ...
                        logging.error(
                            "[ICALabelingStep] No output_file or paths provided, cannot save data"
                        )
                        return data_clean
                    ...
                    logging.info(
                        f"[ICALabelingStep] Saving cleaned data to {clean_file}"
                    )
                    ...
                    logging.error(
                        "[ICALabelingStep] Continuing without saving cleaned data"
                    )
            ...
            if params.get("save_data", False) and params.get("output_file", None):
                try:
                    ...
                    logging.info(
                        f"[ICALabelingStep] Saved data with ICA labels to {params['output_file']}"
                    )
                except Exception as e:
                    logging.error(
                        f"[ICALabelingStep] Error saving data to {params['output_file']}: {e}"
                    )
            if "ipykernel" in sys.modules and self.figures:
                for fig in self.figures:
                    if fig is not None:
    ...
    def _label_with_iclabel(self, ica, data, params):
        ...
        try:
            ...
            if importlib.util.find_spec("mne_icalabel") is None:
                logging.warning(
                    "[ICALabelingStep] mne-icalabel package is not installed. Cannot use ICLabel."
                )
                return None
            import mne_icalabel
            ...
            logging.info(
                f"[ICALabelingStep] ICLabel classification successful: {ic_labels['labels']}"
            )
            return ic_labels
        except Exception as e:
    ...
    def _label_with_correlation(self, ica, data, params):
        ...
        logging.info(
            "[ICALabelingStep] Labeling components by correlation with EOG/ECG"
        )
        result = {}
        ...
        if params["eog_ch_names"]:
            try:
                ...
                if eog_indices:
                    logging.info(
                        f"[ICALabelingStep] Identified EOG components: {eog_indices}"
                    )
                ...
                logging.error(
                    f"[ICALabelingStep] Error identifying EOG components: {e}"
                )
        if params["ecg_channel"] and (params["ecg_channel"] in data.ch_names):
            try:
                ecg_indices, ecg_scores = ica.find_bads_ecg(
                    data, ch_name=params["ecg_channel"], threshold=thresholds["ecg"]
                )
                if ecg_indices:
                    logging.info(
                        f"[ICALabelingStep] Identified ECG components: {ecg_indices}"
                    )
                ...
                logging.error(
                    f"[ICALabelingStep] Error identifying ECG components: {e}"
                )
        return result if result else None

    def _plot_labeled_components(
        self, ica, data, labeled_components, all_artifacts, plot_dir, params
    ):
        """
        ...
        try:
            ...
            ax_overview.set_title(
                "ICA Components Classification Overview", fontsize=14, fontweight="bold"
            )
            ...
            for comp in range(n_components):
                ...
                if "iclabel" in labeled_components and comp < len(
                    labeled_components["iclabel"]["labels"]
                ):
            ...
            ax_overview.bar(
                np.arange(n_components), [1] * n_components, color=comp_colors
            )
            ...
            for comp_idx in sorted(all_artifacts):
                try:
                    fig_props = ica.plot_properties(
                        data, picks=comp_idx, psd_args={"fmax": 50}, show=False
                    )
                    if not isinstance(fig_props, list):
                        fig_props = [fig_props]
                    for i, fig in enumerate(fig_props):
                        comp_file = os.path.join(
                            labeled_dir, f"component_{comp_idx:02d}_properties_{i}.png"
                        )
                        fig.savefig(comp_file, dpi=300)
                        all_figs.append(fig)
                        print(
                            f"[ICALabelingStep] Saved detailed plot for component {comp_idx} to {comp_file}"
                        )
                except Exception as e:
                    logging.error(
                        f"[ICALabelingStep] Error plotting properties for component {comp_idx}: {e}"
                    )
    ...
    def _plot_before_after(self, data_orig, data_clean, ica, plot_dir, params):
        ...
        try:
            ...
            print(
                f"[ICALabelingStep] Saving before/after plots to: {os.path.abspath(ba_dir)}"
            )
            ...
            psds_orig, freqs_orig = data_orig.compute_psd(
                method="welch", n_fft=n_fft_val, average="mean"
            )[:2]
            psds_clean, freqs_clean = data_clean.compute_psd(
                method="welch", n_fft=n_fft_val_clean, average="mean"
            )[:2]
            ...
            ax_psd.semilogy(
                freqs_orig, psds_orig, color="red", alpha=0.8, label="Before"
            )
            ax_psd.semilogy(
                freqs_clean, psds_clean, color="blue", alpha=0.8, label="After"
            )
