...
class EpochingStep(BaseStep):
    ...
    def run(self, data):
        ...
        logging.info(
            f"[EpochingStep.run] Task type: {task_type}, Trigger IDs: {trigger_ids}"
        )
        logging.info(
            f"[EpochingStep.run] Epoch params: tmin={tmin}, tmax={tmax}, baseline={baseline}"
        )

        # Route to task-specific epoching method
        if task_type.lower() == "5pt":
            result = self._epoch_five_point_test(
                data,
                trigger_ids,
                tmin,
                tmax,
            ...
            result = self._epoch_fixed_length(
                data,
                trigger_ids,
                epoch_params,
                reject_by_annotation,
    ...
    def _epoch_five_point_test(
        ...
        logging.info(
            f"[EpochingStep] Epoching 5-point test data using pattern-based approach"
        )
        ...
        events = mne.find_events(
            data, stim_channel=stim_channel, shortest_event=1, verbose=True
        )

        if len(events) == 0:
            logging.error(
                f"[EpochingStep] No events found with stim_channel={stim_channel}. Available channels: {data.ch_names}"
            )
            ...
            if "STI" in data.ch_names and stim_channel != "STI":
                ...
                events = mne.find_events(
                    data, stim_channel="STI", shortest_event=1, verbose=True
                )
        ...
        if len(events) < 3:  # Need at least start, one pair (onset+response), and end
            logging.warning(
                f"[EpochingStep] Not enough events found ({len(events)}), need at least 4"
            )
            return data
        ...
        logging.info(
            f"[EpochingStep] Start event at {start_time:.2f}s, End event at {end_time:.2f}s"
        )
        ...
        start_time_with_buffer = max(
            0, start_time - buffer_pre
        )  # Ensure we don't go below 0
        end_time_with_buffer = min(
            end_time + buffer_post, data.times[-1]
        )  # Don't exceed data length

        # Crop the data
        data_cropped = data.copy().crop(
            tmin=start_time_with_buffer, tmax=end_time_with_buffer
        )
        ...
        if self.params.get("stim_channel", "Trigger") in data_cropped.ch_names:
            ...
            if len(events) >= 2:
                ...
                end_time = end_event[0] / sfreq
                logging.info(
                    f"[EpochingStep] Updated start_time={start_time:.2f}s and end_time={end_time:.2f}s"
                )
            else:
                logging.warning(
                    "[EpochingStep] Could not find enough events in cropped data"
                )
        else:
            logging.warning(
                f"[EpochingStep] Stim channel {self.params.get('stim_channel', 'Trigger')} not found in cropped data"
            )
        ...
        if len(middle_events) > 0:
            ...
            if len(middle_events) % 2 != 0:
                ...
                if onset_events:
                    onset_events.pop()
                    logging.info(
                        "[EpochingStep] Removed last onset event because it has no response (time ran out)"
                    )
        else:
            logging.warning(
                "[EpochingStep] No onset/response events found between start and end triggers"
            )
        ...
        if self.params.get("add_annotations", True):
            ...
            if (
                data_cropped.annotations is not None
                and len(data_cropped.annotations) > 0
            ):
            ...
            if onset_events and response_events:
                ...
                data_cropped.set_annotations(
                    data_cropped.annotations + event_annotations
                )
        ...
        if onset_events and response_events:
            ...
            if response_latencies:
                data_cropped.info["temp"]["5pt_task"]["avg_response_time"] = np.mean(
                    response_latencies
                )
                data_cropped.info["temp"]["5pt_task"]["min_response_time"] = np.min(
                    response_latencies
                )
                data_cropped.info["temp"]["5pt_task"]["max_response_time"] = np.max(
                    response_latencies
                )

        logging.info(
            f"[EpochingStep] Extracted continuous segment from {start_time_with_buffer:.2f}s to {end_time_with_buffer:.2f}s"
        )
        logging.info(
            f"[EpochingStep] Task duration: {end_time - start_time:.2f}s with {len(onset_events)} trials"
        )

        # STEP 2: If extract_continuous is True, return the cropped data without creating epochs
        if extract_continuous:
            logging.info(
                "[EpochingStep] Returning continuous data segment without creating epochs (extract_continuous=True)"
            )
            return data_cropped

        # STEP 3: If extract_continuous is False, create epochs from the cropped data
        logging.info(
            "[EpochingStep] Creating epochs from cropped data (extract_continuous=False)"
        )
        ...
        logging.info(
            f"[EpochingStep] Classified {len(onset_events)} onset events and {len(response_events)} response events"
        )
        ...
        logging.info(
            f"[EpochingStep] Created {len(epochs)} epochs around onset events for 5-point test"
        )
        ...
        if onset_events and response_events:
            try:
                ...
                logging.warning(
                    f"[EpochingStep] Could not calculate response times: {e}"
                )
    ...
    def _epoch_go_nogo(
        ...
        if go_id is None or nogo_id is None or response_id is None:
            logging.error(
                "[EpochingStep] Missing required trigger codes for Go/No-Go task"
            )
            logging.error(
                f"   go_id: {go_id}, nogo_id: {nogo_id}, response_id: {response_id}"
            )
            logging.error(
                "   Please provide 'go', 'nogo', and 'response' in trigger_ids"
            )
            return data
        ...
        # Find events in the data
        events = mne.find_events(
            data, stim_channel=stim_channel, shortest_event=5, min_duration=0.01
        )
        ...
        if go_id not in unique_ids:
            logging.error(
                f"[EpochingStep] Go trigger ID {go_id} not found in the data!"
            )
        if nogo_id not in unique_ids:
            logging.error(
                f"[EpochingStep] NoGo trigger ID {nogo_id} not found in the data!"
            )
        if response_id not in unique_ids:
            logging.error(
                f"[EpochingStep] Response trigger ID {response_id} not found in the data!"
            )
        ...
        if self.params.get("visualize_events", False):
            ...
            if fig:
                ...
                if self.params.get("plot_params", {}).get("save_plots", False):
                    save_dir = self.params.get("plot_params", {}).get(
                        "save_dir", "figures/events"
                    )
        ...
        logging.info(
            f"[EpochingStep] Event counts: {len(go_events)} go, {len(nogo_events)} nogo, {len(response_events)} response"
        )
        ...
        for i in range(len(all_stim_events)):
            ...
            if len(responses_between) > 0:
                ...
                response_time = (responses_between[0][0] - current_time) / data.info[
                    "sfreq"
                ]
                logging.debug(
                    f"Response time for event at {current_time}: {response_time:.3f}s"
                )
        ...
        logging.info(
            f"[EpochingStep] Correct trials: {correct_go_count} go, {correct_nogo_count} nogo"
        )
        ...
        logging.info(
            f"[EpochingStep] Hit rate: {hit_rate:.2f}, Correct NoGo response rate: {correct_response_rate:.2f}"
        )
        ...
        try:
            ...
            logging.info(
                f"[EpochingStep] Created {len(epochs)} epochs for Go/No-Go task with {len(event_id)} conditions"
            )
            logging.info(
                f"[EpochingStep] Available conditions: {list(event_id.keys())}"
            )
    ...
    def _extract_continuous_segment(self, data, trigger_ids, preload):
        ...
        if start_id is None or end_id is None:
            logging.warning(
                "[EpochingStep] Missing start or end trigger IDs for continuous extraction"
            )
            return data
        ...
        if len(events) == 0:
            logging.error(
                f"[EpochingStep] No events found with stim_channel={stim_channel}"
            )
            # Try with STI as a fallback
            if "STI" in data.ch_names and stim_channel != "STI":
                logging.info("[EpochingStep] Trying with STI channel as fallback")
        ...
        if len(start_events) == 0 or len(end_events) == 0:
            logging.warning(
                "[EpochingStep] No start or end events found for continuous extraction"
            )
            return data
        ...
        logging.info(
            f"[EpochingStep] Extracted continuous segment from {start_time:.2f}s to {end_time:.2f}s"
        )
    ...
    def _epoch_custom(
        ...
        logging.info(
            f"[EpochingStep] Created {len(epochs)} epochs with custom trigger configuration"
        )
    ...
    def _epoch_fixed_length(
        ...
        # Log the epoching strategy
        logging.info(
            f"[EpochingStep] Creating fixed-length epochs: duration={duration}s, overlap={overlap}s"
        )
        ...
        if extract_continuous:
            ...
            if len(events) < 2:  # Need at least start and end events
                logging.warning(
                    f"[EpochingStep] Not enough events found ({len(events)}) for task segment detection"
                )
                return data
            ...
            logging.info(
                f"[EpochingStep] Using task segment from {start_time_with_buffer:.2f}s to {end_time_with_buffer:.2f}s"
            )
        else:
            # Use the entire data
    ...
    def plot_epochs(
        ...
        if plot_type == "average":
            ...
            try:
                ...
                for i, (evoked, event_name) in enumerate(zip(evokeds, event_names)):
                    ...
                    if (
                        ...
                        ch_idx = (
                            evoked.ch_names.index(channels[0])
            ...
            if len(event_names) != 2:
                logging.warning(
                    "[EpochingStep] Need exactly 2 event types for difference plot"
                )
                return None
            ...
            try:
                ...
                if title is None:
                    diff_wave.comment = (
                        f"Difference: {event_names[0]} - {event_names[1]}"
                    )
                else:
            ...
            for event_name in event_names:
                try:
                    ...
                    ax.set_title(
                        f"Global Field Power: {event_name}" if title is None else title
                    )
                    plt.tight_layout()
                    figures.append(fig)
                except Exception as e:
    ...
    def plot_events_simple(
        ...
        if stim_channel not in raw.ch_names:
            logging.warning(
                f"[EpochingStep] Stim channel {stim_channel} not found in raw data"
            )
            return None
        ...
        logging.info(
            f"[EpochingStep] Showing {len(visible_events)} events in time window [{tstart:.1f}, {tmax:.1f}]s"
        )
        for i in range(min(5, len(visible_events))):
            logging.info(
                f"  Event {i}: ID={visible_events[i, 2]}, time={visible_times[i]:.3f}s, sample={visible_events[i, 0]}"
            )

        # Get unique event values
        unique_values = (
            np.unique(visible_values)
            if len(visible_values) > 0
            else np.unique(event_values)
        )
        ...
        for i, val in enumerate(unique_values):
            ...
            mask = (
                visible_values == val
                if len(visible_values) > 0
                else event_values == val
            )
            if np.sum(mask) > 0:
                ev_times = (
                    visible_times[mask]
                    if len(visible_values) > 0
                    else event_times[mask]
                )
        ...
        ax.set_title(
            f"Trigger Channel ({stim_channel}) - {duration}s window from {tstart}s to {tmax}s"
        )
        ...
        plt.figtext(
            0.02,
            0.02,
            f"raw.first_samp = {raw.first_samp}",
            fontsize=8,
    ...
    def generate_plots(self, epochs, plot_params=None):
        ...
        if not hasattr(epochs, "event_id"):
            logging.error(
                "[EpochingStep.generate_plots] Input is not a valid Epochs object"
            )
            return None
