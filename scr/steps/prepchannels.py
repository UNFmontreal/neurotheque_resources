...
class PrepChannelsStep(BaseStep):
    ...
    def run(self, data):
        ...
        existing_non_eeg = [
            ch for ch in non_eeg_channels if ch in data.info["ch_names"]
        ]
        ...
        if eeg_channels:
            rename_mapping = {
                ch: ch.replace("EEG ", "").replace("-Pz", "") for ch in eeg_channels
            }
        ...
        logging.info(
            f"Channel types set: {eeg_count} EEG, {stim_count} STIM, {misc_count} MISC"
        )
        ...
        if reference_params:
            logging.info(
                f"[PrepChannelsStep] Applying re-referencing with params: {reference_params}"
            )
            # Default reference parameters for our EEG system
            if not reference_params.get("method"):
                reference_params["method"] = "channels"
            if (
                not reference_params.get("channels")
                and reference_params["method"] == "channels"
            ):
            ...
            try:
                ...
                logging.error(
                    f"[PrepChannelsStep] Error during re-referencing: {str(e)}"
                )
                if on_missing == "raise":
                    raise
                else:
                    logging.info(
                        "[PrepChannelsStep] Continuing without re-referencing (on_missing=ignore)"
                    )
        ...
        try:
            ...
            if on_missing == "raise":
                ...
                logging.info(
                    "[PrepChannelsStep] Continuing without montage (on_missing=ignore)"
                )
