...
class PrepChannelsStep(BaseStep):
    """
    A custom step that drops/renames channels, sets montage, etc.
    Mimics the logic from your notebook (non-EEG channels, rename, montage).
    """

    def run(self, data):
        """
        data : mne.io.Raw (already loaded)
        Returns the same Raw, but with updated channels.
        """
        if data is None:
            raise ValueError("No raw data provided to PrepChannelsStep.")

        # Non-EEG channels to remove
        non_eeg_channels = ['EEG X1:ECG-Pz', 'EEG X2:-Pz', 'EEG X3:-Pz',
