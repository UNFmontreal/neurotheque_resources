...
def get_available_steps():
    ...
    steps = [
        f.stem for f in steps_dir.glob("*.py") if f.stem not in ["__init__", "base"]
    ]
    return steps
...
def main():
    ...
    click.echo(
        "This wizard will help you create a configuration file for your pipeline."
    )
    ...
    config["directory"] = {
        ...
        "raw_data_dir": click.prompt(
            "Enter the raw data directory (relative to root)", default="data/raw"
        ),
        "processed_dir": click.prompt(
            "Enter the processed data directory (relative to root)",
            default="data/processed",
        ),
        "reports_dir": click.prompt(
            "Enter the reports directory (relative to root)", default="reports"
        ),
        "derivatives_dir": click.prompt(
            "Enter the derivatives directory (relative to root)", default="derivatives"
        ),
    }
    ...
    try:
        ...
        if click.confirm(
            "An error occurred during validation. Do you want to save anyway?"
        ):
            pass
        else:
            click.echo("Aborting.")
