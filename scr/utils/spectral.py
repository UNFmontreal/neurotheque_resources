...
def compute_and_plot_psd(
    ...
    psd_params = {"method": method, "fmin": fmin, "fmax": fmax, "picks": picks}
    ...
    if hasattr(data, "compute_psd"):
        ...
        raise ValueError(
            "Data object does not support PSD computation. Ensure you're using a compatible MNE object."
        )
    ...
    if plot_type == "standard":
        ...
        fig, axes = plt.subplots(
            1, n_freqs, figsize=plot_kwargs.pop("figsize", (4 * n_freqs, 4))
        )
        if n_freqs == 1:
            axes = [axes]
        ...
        for band_name, (band_min, band_max) in bands.items():
            ...
            if len(band_indices) == 0:
                logging.warning(
                    f"No frequencies found in {band_name} band ({band_min}-{band_max} Hz)"
                )
                continue
        ...
        avg_powers = [
            np.mean(band_powers[band]) for band in bands.keys() if band in band_powers
        ]
        ...
        if picks is not None and not isinstance(picks, str):
            ...
            ch_idx = mne.pick_types(
                data.info,
                **{p: True for p in ("all",) if p == picks} if picks else {"all": True},
            )
            channel_names = (
                [data.info["ch_names"][i] for i in ch_idx]
    ...
    if show:
        ...
        if backend.lower() in ("agg", "figurecanvasagg"):
            try:
                ...
                logging.warning(
                    "Plot display might fail. Consider setting matplotlib backend manually."
                )
        plt.show()
...
def plot_time_frequency(
    ...
    # Set up frequencies
    freqs = np.logspace(np.log10(freq_range[0]), np.log10(freq_range[1]), num=20)
    ...
    if isinstance(data, mne.io.Raw):
        ...
        epochs = mne.Epochs(
            data, events, tmin=0, tmax=1.0, baseline=None, preload=True, picks=picks
        )
    ...
    if plot_type == "power" or plot_type == "both":
        ...
        if save_path and plot_type == "power":
            power_path = (
                save_path.replace(".png", "_power.png")
                if "." in save_path
                else f"{save_path}_power.png"
            )
    ...
    if plot_type == "itc" or plot_type == "both":
        ...
        if save_path and plot_type == "itc":
            itc_path = (
                save_path.replace(".png", "_itc.png")
                if "." in save_path
                else f"{save_path}_itc.png"
            )
