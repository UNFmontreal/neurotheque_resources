...
def find_autoreject_log(
    subject_id,
    session_id=None,
    task_id=None,
    run_id=None,
    base_dir=None,
    search_derivatives=True,
):
    """
    ...
    if task_id and run_id:
        filenames.append(
            f"sub-{subject_id}_ses-{session_id}_task-{task_id}_run-{run_id}_autoreject_log.pickle"
        )
    elif task_id:
        filenames.append(
            f"sub-{subject_id}_ses-{session_id}_task-{task_id}_autoreject_log.pickle"
        )
    else:
...
def load_autoreject_log(
    ...
    if filepath is None:
        if subject_id is None or session_id is None:
            raise ValueError(
                "If filepath is not provided, subject_id and session_id must be provided"
            )

        filepath = find_autoreject_log(
            subject_id, session_id, task_id, run_id, base_dir
        )

        if filepath is None:
            print(
                f"Could not find autoreject log for sub-{subject_id} ses-{session_id}"
            )
            return None
...
def plot_autoreject_summary(reject_log, figsize=(20, 16), save_to=None):
    ...
    problematic_channels = [
        (ch, rate) for ch, rate in zip(ch_names, channel_rejection_rates) if rate > 50
    ]

    # Create a comprehensive figure with subplots
    fig = plt.figure(figsize=figsize)
    gs = plt.GridSpec(
        2, 2, height_ratios=[3, 1], width_ratios=[3, 1], wspace=0.15, hspace=0.25
    )

    # Main heatmap - improved visualization
    ax_main = fig.add_subplot(gs[0, 0])
    im = ax_main.imshow(
        mask, cmap="viridis", aspect="auto", interpolation="nearest", vmin=0, vmax=2
    )
    ...
    bars = ax_ep.bar(
        range(n_epochs), [1 if x else 0 for x in bad_epochs], color="red", alpha=0.7
    )
...
def plot_reject_log(
    reject_log, orientation="horizontal", figsize=(18, 14), save_to=None
):
    """
    ...
    try:
        ...
        original_fig = reject_log.plot(
            orientation=orientation, show_names=True, show=False
        )
        ...
        plt.suptitle(
            "AutoReject Channel Status", fontsize=16, fontweight="bold", y=0.98
        )
        ...
        plt.text(
            0.5,
            0.5,
            "Could not create enhanced RejectLog plot. See detailed visualization instead.",
